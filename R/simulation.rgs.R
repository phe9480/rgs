...
#' then the rgs object's weighting strategy will be ignored. fws can contain multiple weighting strategies.
...
#' @param Cox "Y" or "N" to indicate whether Cox regression model is requested to produce HR estimate
...
#' 
#' @export  
simulation.rgs = function(nSim=10000, N = 672, A = 21, w=1.5, r=1,
                          lambda0=log(2)/12, lambda1=log(2)/12*0.65,
                          cuts=NULL, dropOff0=0, dropOff1=0, 
  ...
  #if type1err is not provided, use sf to derive type1err 
  #if type1err is provided, then sf is ignored.
  side = 1 #always
  ld.obf = function(s){
  ...
  CutOff = hr = med0 = med1 = matrix(NA, nrow=nSim, ncol = K)
  
  #(1). Generate data 

  for (i in 1:nSim) {    
    if (is.null(scanfreq0) || is.null(scanfreq1)) {
      dati = simulation.pwexp(nSim=1, N = N, A = A, w=w, r=r, lam0=lambda0, lam1=lambda1, 
                             cuts=cuts, drop0=dropOff0, drop1=dropOff1, 
                             targetEvents = targetEvents, DCO = DCO)
    } else{
      dati = simulation.pwexp.pfs(nSim=1, N = N, A = A, w=w, r=r, lam0=lambda0, lam1=lambda1, 
                                 cuts=cuts, drop0=dropOff0, drop1=dropOff1, 
                                 targetEvents = targetEvents, DCO = DCO, 
