#' Simulate PFS data following piece-wise exponential distribution with non-uniform accrual pattern and lost to follow-up considering scan intervals
#'
...
#' 
#' @export 
simulation.pwexp.pfs = function(nSim=100, N = 600, A = 21, w=1.5, r=1, 
                            lam0=log(2)/12, lam1=log(2)/12*0.65, cuts=NULL, 
                            drop0=0, drop1=0, targetEvents = c(400, 500), 
                            DCO = NULL, scanfreq0=c(rep(6, 2), rep(9, 100))*7/(365.25/12), 
                            scanfreq1=c(rep(6, 2), rep(9, 100))*7/(365.25/12)) {
  ...
  for (j in 1:nSim) {
    ...
    #PFS scan interval
    dataj$survTime=pfs_scan(time = dataj$survTime, cnsr = dataj$cnsr, 
                        treatment = dataj$treatment, 
