...
#' x = pmcr(t=12, p=0.15, alpha = log(2)/12, beta=1, gamma=1, lambda=0)
#' 
#' t=seq(1,100)
#' c = sapply(t, pmcr, p=0.15, alpha = log(2)/12, beta=1, gamma=1, lambda=0)
#' plot(t, 1-c, type="l", ylab="", main="Survival Function")
#' 
#' #(2) Weibull distribution mixture with cure rate 0.15
#' p = 0.15; alpha = log(2)/12
#' x = pmcr(t=12, p=0.15, alpha = log(2)/12, beta=1, gamma=0.9, lambda=0)
#' 
#' #(3) Rayleigh distribution mixture with cure rate 0.15
#' p = 0.15; alpha = log(2)/100
#' x = pmcr(t=12, p=0.15, alpha = alpha, beta=1, gamma=2, lambda=0)
#' 
#' #(4) GMW distribution mixture with cure rate 0.15
#' p = 0.15; alpha = log(2)/12
#' x = pmcr(t = 12, p = 0.15, alpha = alpha, beta=1, gamma=1, lambda=0.2)
#' 
#' @export
#' 
