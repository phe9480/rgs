#' Survival Function for Mixture Cure Rate Distribution
#' 
...
#' x = smcr(t=12, p=0.15, alpha = log(2)/12, beta=1, gamma=1, lambda=0)
#' 
#' t=seq(1,100)
#' c = sapply(t, smcr, p=0.15, alpha = log(2)/12, beta=1, gamma=1, lambda=0)
#' plot(t, c, type="l", ylab="", main="Survival Function")
#' 
#' #(2) Weibull distribution mixture with cure rate 0.15
#' p = 0.15; alpha = log(2)/12
#' x = smcr(t=12, p=0.15, alpha = log(2)/12, beta=1, gamma=0.9, lambda=0)
#' 
#' #(3) Rayleigh distribution mixture with cure rate 0.15
#' p = 0.15; alpha = log(2)/100
#' x = smcr(t=12, p=0.15, alpha = alpha, beta=1, gamma=2, lambda=0)
#' 
#' #(4) GMW distribution mixture with cure rate 0.15
#' p = 0.15; alpha = log(2)/12
#' x = smcr(t = 12, p = 0.15, alpha = alpha, beta=1, gamma=1, lambda=0.2)
#' 
#' @export
#' 
smcr = function(t, p=0.3, alpha = log(2)/12, beta=1, gamma=1, lambda=0, tau=0, psi=1) {
  1 - pmcr(t, p, alpha, beta, gamma, lambda, tau, psi)
}
