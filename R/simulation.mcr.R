#' Simulation of Two-Arm Trial Data Following Mixture Cure Rate Distribution
#' 
...
simulation.mcr = function(nSim=100, N = 600, A = 18, w=1.5, r=1, p=c(0.1,0.1), 
  ...
  f.dataCut = function(data, targetEvents = 397, DCO = NULL) {
    ...
    if(is.null(DCO)){
      ...
      ##special consideration for MCR: If the simulated data has events < targetEvents, set to Infinity
      if (max(data.event$event.seq) < targetEvents){
        data0$calendarCutoff = Inf
      } else{
  ...
  for (i in 1:nSim){
    ...
    dati.cut = NULL
    for (ii in 1:L){
      out[[ii]] = f.dataCut(data=dati, targetEvents=targetEvents[ii], DCO = DCO[ii])
    }
  }
  return(out)
}
